...
public class AStarSearchPathWithCos2 implements AStar{
	...
	public final static int DIRECTCONTS=10;
	...
	public void addNeighborsToOpenProcess(MapInit map, Node current, int x, int y, int roadCost) {
		...
		if(canNodeReach(map,x,y)&&!isInCloseTable(x,y)){
			...
			*/
			...
			if(child==null){
				float h=cal_H(current.getPosition(),end.getPosition(),position,null);
